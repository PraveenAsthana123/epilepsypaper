...
\textbf{Methods.} Holding feature representation and classifier fixed, we change the validation protocol. The key control is \emph{within} CHB-MIT (identical 20-D features and balanced Random Forest, epoch-level 5-fold vs strict Leave-One-Subject-Out over 24 subjects), so the protocol effect is not confounded by dataset; a Bonn/UCI epoch-level point (11{,}500 segments) is reported separately as a protocol-plus-dataset reference. We report the full metric set (sensitivity, specificity, PPV/NPV, F1, MCC, ROC-/PR-AUC) pooled and macro per-subject, with subject-level bootstrap CIs.
\textbf{Results.} On the \emph{same} CHB-MIT data, switching epoch-level$\rightarrow$LOSO cuts macro sensitivity 48.6\%$\rightarrow$35.1\% (pooled 49.8\%$\rightarrow$27.5\%), nearly halves MCC (0.64$\rightarrow$0.35) and PR-AUC (0.79$\rightarrow$0.40), while specificity stays high (97\%). The cross-dataset Bonn/UCI epoch-level point (88.4\% sensitivity) reflects additional dataset difficulty.
...
\end{tabular}\end{table}
\textbf{Pooled vs macro.} We report \emph{both} the pooled-confusion sensitivity (27.5\%, all epochs pooled) and the macro per-subject mean (35.1\%, the headline); their gap reflects between-subject variance, disclosed fully in Table~\ref{tab:full}. \textbf{Clinical operating points.} Under LOSO, sensitivity at fixed specificity is 46\% @ 90\%, 36\% @ 95\%, and 19\% @ 99\% (Table~\ref{tab:oppt})---honest numbers a clinician can act on.
\begin{table}[h]\centering\caption{LOSO sensitivity at fixed specificity (same-dataset CHB-MIT).}\label{tab:oppt}
\begin{tabular}{lc}\toprule
Fixed specificity & Sensitivity\\\midrule
90\% & 0.46\\
95\% & 0.36\\
99\% & 0.19\\\bottomrule
\end{tabular}\end{table}
...
To show the leakage gap is model-independent, we evaluate statistical, classical-ML, and a deep MLP baseline under the \emph{same} CHB-MIT LOSO split on the identical 20-D features (Table~\ref{tab:bench}). No family exceeds $\sim$56\% LOSO sensitivity; the high-specificity/low-sensitivity profile is universal. Cost-sensitive Logistic Regression trades accuracy for the highest LOSO sensitivity (69\%); ensembles maximise specificity but suppress sensitivity. Standardisation is fit on the training fold only (no scaling leakage), and feature selection (Table earlier) is validated subject-wise.
...
\end{tabular}\end{table}
Fig.~\ref{fig:persub} shows per-subject LOSO performance (0--90\% spread); Fig.~\ref{fig:cmc} the aggregate confusion matrix.
...
\section{Deployment Implications: the RGAIG Framework}\label{sec:rgaig}
\textbf{Definition.} RGAIG denotes a \emph{Responsible, Governed, Agentic, Interpretable, retrieval-Grounded} deployment framework. \textbf{Why it is needed.} The honest patient-independent sensitivity (35\%) is, by itself, below the bar for autonomous clinical use; rather than discard the detector, RGAIG converts that honest estimate into safe clinical value by making the model's uncertainty \emph{actionable}---high-confidence cases are auto-cleared, mid-confidence cases escalate to a human, low-confidence cases are rejected. \textbf{How it adds value.} It surrounds the detector with (R) responsible-AI guardrails, (G) a governance/audit trail, (A) an agentic Author$\rightarrow$Reviewer$\rightarrow$Chair workflow over permission-scoped MCP tools, (I) interpretable per-prediction explanations, and (G) retrieval-grounded, citation-backed evidence---so a 35\%-sensitivity model is deployed as triage-with-escalation, not as an unattended decision-maker. We present RGAIG as a \emph{deployment implication}, not as an evaluated empirical contribution; its KPIs (Table~\ref{tab:kpi}) define what a future system study would measure.
We instantiate it as a five-layer stack: (L1) preprocessing; (L2) model; (L3) retrieval-augmented, citation-grounded explanation; (L4) multi-agent governance with permission-scoped MCP tools; (L5) AIOps --- per-subject sensitivity, calibration (ECE/Brier), drift (PSI), an immutable audit row per prediction, a fairness gate (disparate impact $\geq0.8$), and a kill-switch.
\begin{table}[h]\centering\caption{RGAIG governance KPIs (targets for a future deployment study; not evaluated here).}\label{tab:kpi}
\begin{tabular}{lll}\toprule
Letter & KPI & Target\\\midrule
R & Disparate impact (subgroup) & $\geq 0.8$\\
G & Audit-row completeness & 100\%\\
G & Data drift (PSI) & alert if $>0.25$\\
A & Escalation precision & maximise\\
I & Calibration (ECE / Brier) & minimise\\
\,Grounded & Citation-support rate & $\geq 0.95$\\
--- & False alarms / hour & minimise\\
--- & Worst-subject sensitivity & monitor (floor)\\\bottomrule
\end{tabular}\end{table}
Our governance script reports a live PSI drift of 0.66 between early and late subjects (alert-worthy) and a per-subject sensitivity monitor; a full system benchmark (latency, RAG faithfulness, audit demonstration) is future work.
...
All code, per-subject results, and the protocol-comparison artefacts are released at
\texttt{github.com/PraveenAsthana123/epilepsypaper}. Datasets used (all public, all real):
CHB-MIT (PhysioNet, 24 paediatric subjects), Bonn/UCI Epileptic Seizure Recognition
(11{,}500 segments), and Siena Scalp EEG (adult; pipeline released, cross-dataset result
listed as future work). Every number traces to a committed script and result JSON; seed fixed at 42.
...
Pooled + macro metrics, MCC, PR-AUC reported & yes\\
Subject-level bootstrap CIs & yes\\
Fixed seed (42), public datasets only & yes\\\bottomrule
\end{tabular}\end{table*}

\section{Limitations}
(1) CHB-MIT is \emph{paediatric}; adult generalisation (Siena/TUH) is not yet validated and is future work. (2) The Bonn/UCI-vs-CHB-MIT contrast mixes protocol with dataset; we therefore foreground the same-dataset CHB-MIT control (Sec.~\ref{sec:samedataset}). (3) Deployable LOSO sensitivity (35\%) is low; we report it honestly rather than tuning on test folds. (4) RGAIG is specified and partially instrumented but \emph{not} evaluated as a system (no latency, RAG-faithfulness, or audit user study). (5) No prospective clinical trial; all data are retrospective and public. (6) Hyperparameter/threshold choices are made subject-wise; any random-split deep-tuning results are explicitly non-deployable.
