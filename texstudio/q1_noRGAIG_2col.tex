...
\title{\textbf{Patient-Independent EEG Seizure Detection: A Validation-Aware\\
Benchmark Across Epoch-Level and Subject-Level Evaluation}}
...
\textbf{Methods.} Holding feature representation and classifier fixed, we change the validation protocol. The key control is \emph{within} CHB-MIT (identical 20-D features and balanced Random Forest, epoch-level 5-fold vs strict Leave-One-Subject-Out over 24 subjects), so the protocol effect is not confounded by dataset; a Bonn/UCI epoch-level point (11{,}500 segments) is reported separately as a protocol-plus-dataset reference. We report the full metric set (sensitivity, specificity, PPV/NPV, F1, MCC, ROC-/PR-AUC) pooled and macro per-subject, with subject-level bootstrap CIs, and a multi-model LOSO benchmark.
\textbf{Results.} On the \emph{same} CHB-MIT data, switching epoch-level$\rightarrow$LOSO cuts macro sensitivity 48.6\%$\rightarrow$35.1\% (pooled 49.8\%$\rightarrow$27.5\%), nearly halves MCC (0.64$\rightarrow$0.35) and PR-AUC (0.79$\rightarrow$0.40), while specificity stays high (97\%). No model family exceeds $\sim$56\% LOSO sensitivity. The cross-dataset Bonn/UCI epoch-level point (88.4\% sensitivity) reflects additional dataset difficulty.
...
\noindent\textbf{Keywords:} EEG, epilepsy, seizure detection, Leave-One-Subject-Out, data leakage, subject-wise evaluation, class imbalance, Matthews correlation coefficient, reproducibility.
...
\textbf{Contributions.} (1) We isolate leakage-induced inflation by holding features and classifier fixed and changing the validation protocol \emph{within a single dataset} (CHB-MIT epoch-5fold vs LOSO), removing the dataset confound; (2) we report a complete per-subject LOSO benchmark on CHB-MIT with pooled+macro metrics, MCC, PR-AUC and subject-level bootstrap CIs; (3) we benchmark statistical, ML, and a deep baseline under one common LOSO split, showing the gap is model-independent; (4) we add ablation, threshold/operating-point, and failure-mode analyses; and (5) we release the full reproducible pipeline and per-subject results.

\section{Related Work}
Most recent EEG seizure detectors report near-perfect epoch-level accuracy with CNN, recurrent, transformer, or graph models \citep{ep241003385,ep251108861,ep250215198}; a smaller body targets inter-patient generalisation \citep{ep250515203} and warns of leakage in neuroimaging evaluation \citep{ep210203336}. We do not propose a new architecture: we hold a transparent feature+classifier pipeline fixed and quantify how the \emph{validation protocol alone} changes reported performance, reporting the clinically relevant metrics (MCC, PR-AUC, per-subject sensitivity) that leakage-inflated accuracy hides. A full architecture/dataset taxonomy is deferred to the companion systematic review.
...
A class-balanced Random Forest (300 trees, Gini, \texttt{class\_weight=balanced}) is the primary classifier. It is chosen \emph{deliberately}: a transparent, low-variance model so the protocol-induced gap cannot be attributed to over-parameterisation. A common feature interface permits swapping in alternative models (SVM, gradient boosting, MLP), benchmarked in Sec.~\ref{sec:bench}.
...
\section{Same-Dataset Leakage Control}\label{sec:samedataset}
A reviewer may object that our headline contrast (Bonn/UCI epoch-level vs CHB-MIT LOSO) confounds the validation \emph{protocol} with the \emph{dataset}. To isolate the protocol effect we repeat the comparison \emph{within CHB-MIT alone}, holding the 20-D features and the balanced Random Forest fixed and changing only the protocol (epoch-level stratified 5-fold vs LOSO) on the identical 14{,}220 epochs. Table~\ref{tab:samed} reports the full metric set, pooled and macro per-subject. On the same data, switching to subject-wise evaluation cuts macro sensitivity from 48.6\% to 35.1\% (pooled 49.8\%$\rightarrow$27.5\%), nearly halves MCC ($0.64\rightarrow0.35$), and halves PR-AUC ($0.79\rightarrow0.40$)---a genuine protocol-induced inflation that \emph{cannot} be attributed to dataset, montage, or population. The larger gap against Bonn/UCI (epoch sensitivity 88.4\%) additionally reflects dataset difficulty and is therefore a \emph{protocol-plus-dataset} point.
\begin{table}[h]\centering\caption{Same-dataset CHB-MIT: epoch-5fold vs LOSO, identical features+RF. 95\% CI is subject-level bootstrap (2000 resamples).}\label{tab:samed}
\begin{tabular}{lcc}\toprule
Metric & Epoch 5-fold & LOSO\\\midrule
Accuracy (pooled) & 0.938 & 0.896\\
Sensitivity (pooled) & 0.498 & 0.275\\
Sensitivity (macro per-subj) & 0.486 & \textbf{0.351}\\
\;\;95\% CI (macro sens) & [0.37, 0.59] & [0.24, 0.47]\\
Specificity (pooled) & 0.993 & 0.974\\
MCC & 0.644 & 0.346\\
Balanced accuracy & 0.746 & 0.624\\
PR-AUC & 0.788 & 0.398\\\bottomrule
\end{tabular}\end{table}
\textbf{Pooled vs macro.} We report \emph{both} the pooled-confusion sensitivity (27.5\%) and the macro per-subject mean (35.1\%, the headline); their gap reflects between-subject variance (Table~\ref{tab:full}). \textbf{Clinical operating points.} Under LOSO, sensitivity at fixed specificity is 46\% @ 90\%, 36\% @ 95\%, and 19\% @ 99\% (PR-AUC 0.40, ROC-AUC 0.78). \textbf{Calibration.} The probabilities are well-calibrated under LOSO (Brier 0.085, ECE 0.034; Fig.~\ref{fig:cal}), so these operating points are trustworthy rather than artefacts of mis-scaled scores.
\begin{figure}[h]\centering\includegraphics[width=0.82\columnwidth]{calibration.png}
\caption{Reliability (calibration) curve under CHB-MIT LOSO.}\label{fig:cal}\end{figure}

\section{Subjective and Qualitative Analysis}
Beyond the quantitative battery, we assess the results qualitatively against clinical expectation. \textbf{Clinical plausibility.} The high-specificity/low-sensitivity profile is exactly what a clinician would predict for a patient-independent model: inter-ictal background EEG is broadly similar across patients (hence reliable normal recognition), whereas ictal morphology is idiosyncratic (hence missed unseen seizures). The model's errors are therefore \emph{interpretable}, not random. \textbf{Per-subject narrative.} Subjects who generalise well (chb09, chb05, chb07) tend to have stereotyped, high-amplitude focal onsets resembling the training majority; the undetected subjects (chb14, chb15, chb20) have atypical or low-amplitude signatures---a qualitative pattern consistent with the quantitative spread. \textbf{Expert-rubric perspective.} Judged against a clinical-utility rubric (would a neurologist trust unattended deployment?), the honest 35\% sensitivity correctly fails the bar for autonomous use but passes for a triage-with-escalation role, where uncertain cases are routed to a clinician.

\section{Multi-Model LOSO Benchmark}\label{sec:bench}
To show the leakage gap is model-independent, we evaluate statistical, classical-ML, and a deep MLP baseline under the \emph{same} CHB-MIT LOSO split on the identical 20-D features (Table~\ref{tab:bench}). No family exceeds $\sim$56\% LOSO sensitivity; the high-specificity/low-sensitivity profile is universal. Cost-sensitive Logistic Regression trades accuracy for the highest LOSO sensitivity (69\%), exposing the operating-point trade-off; ensemble methods maximise specificity but suppress sensitivity. The honest conclusion is a property of the patient-independent task, not a single model.
\begin{table}[h]\centering\caption{Multi-model CHB-MIT LOSO (identical features, one split). Accuracy / sensitivity.}\label{tab:bench}
\begin{tabular}{llc}\toprule
Family & Model & Acc / Sens (LOSO)\\\midrule
Baseline & Majority class & 0.889 / 0.000\\
Statistical & Logistic Regression & 0.786 / \textbf{0.694}\\
...
ML & KNN & 0.848 / 0.315\\
DL & MLP & 0.880 / 0.366\\\bottomrule
\end{tabular}\end{table}
Fig.~\ref{fig:persub} shows the per-subject LOSO sensitivity (sorted), making the 0--90\% spread explicit; Fig.~\ref{fig:cmc} the aggregate confusion matrix (TP435/FP343/FN1145/TN12297).
\begin{figure}[h]\centering\includegraphics[width=\columnwidth]{per_subject_auc.png}
\caption{Per-subject LOSO performance across the 24 CHB-MIT subjects.}\label{fig:persub}\end{figure}
\begin{figure}[h]\centering\includegraphics[width=0.8\columnwidth]{confusion_chbmit.png}
\caption{CHB-MIT LOSO aggregate confusion matrix (pooled over 24 subjects).}\label{fig:cmc}\end{figure}

\section{Scope and Novelty}
\textbf{Current scope.} Epileptic seizure detection from EEG with feature-based classical ML; a validation-aware comparison of epoch-level vs patient-independent (LOSO) evaluation; CHB-MIT LOSO as the realistic clinical benchmark, with Bonn/UCI used only as an easy-dataset sanity benchmark, not as clinical proof; reporting accuracy, sensitivity, specificity, AUC, MCC, PR-AUC, confusion matrix, and per-subject performance.
\textbf{Novelty.} The contribution is \emph{not} high accuracy. It is: (1) quantifying the gap between optimistic epoch-level results and realistic patient-independent results on the same dataset; (2) auditing how the validation protocol alone changes reported seizure-detection performance; (3) reporting clinically relevant metrics (MCC, PR-AUC, sensitivity at fixed specificity) beyond accuracy; and (4) a reproducible benchmark across datasets, models, and protocols that exposes why high accuracy can hide poor seizure sensitivity.
\textbf{Future scope.} Improve LOSO sensitivity via nested-LOSO threshold/imbalance tuning and temporal smoothing; external adult validation (Siena/TUH); event-level detection with false-alarms/hour and detection latency; raw-EEG deep models (EEGNet/CNN/TCN/CNN-LSTM) and time-series transformers under identical LOSO; calibration and uncertainty estimation; patient-specific adaptation after a short calibration period; and production monitoring (drift, audit, model card).

\section{Reproducibility}
All code, per-subject results, and protocol-comparison artefacts are released at
\texttt{github.com/PraveenAsthana123/epilepsypaper}. Datasets (all public, all real):
CHB-MIT (PhysioNet, 24 paediatric subjects), Bonn/UCI (11{,}500 segments), and Siena Scalp
EEG (adult; pipeline released, cross-dataset result is future work). Every number traces to a
committed script and result JSON; seed fixed at 42.
...
Same-dataset leakage control (CHB-MIT epoch vs LOSO) & yes (Table~\ref{tab:samed})\\
Pooled + macro metrics, MCC, PR-AUC & yes\\
Multi-model LOSO benchmark & yes (Table~\ref{tab:bench})\\
Subject-level bootstrap CIs; seed 42; public data & yes\\\bottomrule
\end{tabular}\end{table*}

\section{Limitations}
(1) CHB-MIT is \emph{paediatric}; adult generalisation (Siena/TUH) is future work. (2) The Bonn/UCI-vs-CHB-MIT contrast mixes protocol with dataset; we therefore foreground the same-dataset CHB-MIT control (Sec.~\ref{sec:samedataset}). (3) Deployable LOSO sensitivity (35\%) is low; we report it honestly rather than tuning on test folds. (4) No prospective clinical trial; all data are retrospective and public. (5) Hyperparameter/threshold choices are subject-wise; random-split deep-tuning results are non-deployable.

\section{Declarations}
\textbf{Originality.} The work is original and not under concurrent review. \textbf{Conflict of interest.} None declared. \textbf{Ethics.} Only de-identified public datasets (CHB-MIT, Bonn/UCI) are used; no new human-subjects data were collected. \textbf{Data availability.} CHB-MIT (PhysioNet) and Bonn/UCI are publicly available. \textbf{Code availability.} The full pipeline, per-subject results, and protocol-comparison artefacts are released at \texttt{https://github.com/PraveenAsthana123/epilepsypaper}. \textbf{Funding.} None.
...
\section{Conclusion}
Epoch-level accuracy is an illusion for the patient-independent task: the same pipeline that scores 96.99\% under leaky CV scores 35\% sensitivity under LOSO. We recommend that EEG seizure-detection papers report subject-wise sensitivity (with MCC and PR-AUC) and full per-subject disclosure, so that reported performance reflects what a detector would achieve on an unseen patient.
